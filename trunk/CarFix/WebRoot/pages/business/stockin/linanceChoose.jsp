...
<html>
	...
	<script type="text/javascript">
		...
		function tbPartInfoTableE3ConfigHandler(pConfig) {
			...
			pConfig.form = "chooseTbPartInfoLianceAction";
		...
		function addPartInfo(){
			...
			for(var i=0; i<chpartinfo.length; i++){
				...
				if(!flag){
					...
					dataCollection.push(Math.round(numDom.value*100)/100);				//数量 		length-5
					dataCollection.push(Math.round(costprice.value*100)/100);			//成本价		length-4
					dataCollection.push(Math.round(costtotal*100)/100);					//成本价小计
					dataCollection.push(Math.round(price.value*100)/100);				//进货价		length-2
					dataCollection.push(Math.round(pricetotal*100)/100);				//进货价小计
	...
	<body  >
		...
		<s:form action="chooseTbPartInfoLianceAction.action"   >
		...
		<e3t:table id="tbPartInfoTable" uri="chooseTbPartInfoLianceAction.action" var="tbPartInfo"
			...
			<e3t:column property="num" title="配件数量" >
				<input type="text" maxlength="80" style="width:80px"  id="${tbPartInfo.id}num" />
			</e3t:column>
			<e3t:column property="price" title="${typeName}价" >
			
			<s:if test="#request.typeName == '进货'">
				<input type="text" style="width:80px"  maxlength="80" id="${tbPartInfo.id}price" value="${tbPartInfo.costPrice}" />
			</s:if>
			<s:if test="#request.typeName == '借出'">
				<input type="text" disabled="disabled" style="width:80px"  maxlength="80" id="${tbPartInfo.id}price" value="${tbPartInfo.sellPrice}" />
			</s:if>
			</e3t:column>
