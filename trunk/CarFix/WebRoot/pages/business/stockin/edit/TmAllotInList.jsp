...
<html>
	...
	<script type="text/javascript">
		...
		function tmStockInTableConfigHandler(pConfig) {
			...
			{	
				...
				iconCls : 'addIcon',
				handler : function() {
					//window.location.href = ctx+"/initAllotInPageAction.action";
					addObject('initAllotInPageAction.action',800,450);
				}
			}, '', '-', '', {
				text : '刷新',
				iconCls : 'refreshIcon',
		...
		function tmStockInTableE3ConfigHandler(pConfig) {
			...
			pConfig.form = "findAllotInAction";
	...
	<body>
		...
		<e3t:table id="tmStockInTable" uri="findAllotInAction.action" var="stockIn"
			scope="request" items="stockInList" mode="ajax"  caption="调拨入库单列表"
			...
			<e3t:column property="oucherNo"  title="凭证号" />
			...
			<e3t:column property="no" title="操作"
				sortable="false">
				<a href="javascript:editObject('${stockIn.id}','initEditAllotPageAction.action',800,450)">
					<font color="blue">
						修改
					</font>
				</a>
				...
				&nbsp;&nbsp;
				<a href="javascript:editObject('${stockIn.id}','printDBJCAction.action',600,300);">
					<font color="blue">
						打印
					</font>
				</a>
