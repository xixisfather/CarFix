	...
	function createGrid(){
		...
		var cm = new Ext.grid.ColumnModel([
			...
			{header:'产地',dataIndex:'productArea',editor : new Ext.form.TextField({allowBlank: false}),sortable:true,hidden:true},
	        ...
	        {header:'领用价',dataIndex:'price',editor : new Ext.form.NumberField({allowBlank: false}),sortable:true},
	        {header:'含税单价',dataIndex:'sellPrice',editor : new Ext.form.NumberField({allowBlank: false}),sortable:true,hidden:true},
			...
			afteredit : function(e){
				...
				for(var v in data){
					if( data[v][0] != undefined && data[v][0] == tbPartInfoId){
						...
						data[v][9] = formatFloat(data[v][8],2);
						break;
					}
				}
		...
		function insertAction(confirm){
			...
			if( document.getElementById("userRealName").value == ""){
				Ext.MessageBox.alert('温馨提示：', '领用人不能为空.', null);
				return;
			}
			...
			for(var i=0 ; i< ds.getCount() ; i++){
				...
				var inputValue = partQuantity+"_"+price;
	...
	function addObj(partInfoObj){
		...
		data.unshift([
	       ...
	       formatFloat(partInfoObj.costPrice,2)							//含税价
	     ]
	    );
