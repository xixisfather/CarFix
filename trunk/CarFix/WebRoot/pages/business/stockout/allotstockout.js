...
//打开配件列表窗口
function showWin(){
	var url = "chooseTbPartInfoStockOutAction.action?typeName="+encodeURIComponent(stockOutTypeName);
...
function createGrid(){
    ...
    grid = new Ext.grid.GridPanel({
        ...
        columns: [
            ...
            {header: "成本价", width: 100, sortable: true, hidden:true, dataIndex:'costprice'},
            {header: "成本金额", width: 100, sortable: true,hidden:true, dataIndex:'costtotal'},
...
function addPartCollection(confirm){
	...
	if( document.getElementById("stockOutDate").value == ""){
		Ext.MessageBox.alert('温馨提示：', '出库日期不能为空.', null);
		return;
	}
