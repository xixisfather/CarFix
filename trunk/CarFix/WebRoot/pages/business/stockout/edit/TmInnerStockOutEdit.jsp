...
<html>
	...
	<body>
		...
		<s:form action="updateInnerStockOutAction.action" >
			...
			<s:hidden name="tmStockOut.stockOutCode"  ></s:hidden>
			<div id="fatherId" ></div>
			<table>
				<tr>
					<td>领用日期：</td>
					<td><s:textfield name="tmStockOut.stockOutDate" id="stockOutDate"  >
		...
		<s:iterator id="det" value="#request.detVos" >
			<script type="text/javascript">
				data.push([
			       ...
			       "${det.quantity}",									//入库数量
		       	   "${det.price}",										//成本价
		       	   formatFloat("${det.costPrice}",2)				   //含税价
			     ]
			    );
				...
				document.getElementById("userRealName").value = "${drawPeople.userRealName}";
			</script>
		</s:iterator>
