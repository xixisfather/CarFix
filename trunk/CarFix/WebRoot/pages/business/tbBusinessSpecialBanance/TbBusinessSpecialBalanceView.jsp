...
<html>
	...
	<script language="javascript">
		...
		function tbBusinessSpecialBalanceTableConfigHandler(pConfig) {
...

			{
				...
				iconCls : 'refreshIcon',
				handler : function() {
				window.open('<%=request.getContextPath() %>/pages/business/tbBusinessSpecialBanance/TbBusinessSpecialBalanceGroupPrint.jsp');
				}
			} ];
		...
		function tbBusinessSpecialBalanceTableE3ConfigHandler(pConfig) {
			...
			pConfig.form = "tbBusinessSpecialBalanceViewAction";
	...
	<% 
	      ...
	      //request.setAttribute("comName", "xtl");
	    %>
		<s:form action="tbBusinessSpecialBalanceViewAction.action">
			<table>
				<tr>
					...
					<td>
						<s:textfield name="tbBusinessSpecialBalance.balanceCode"></s:textfield>
					</td>
					<td>
						委托书号
					</td>
		...
		<e3t:table id="tbBusinessSpecialBalanceTable" uri="tbBusinessSpecialBalanceViewAction.action" var="tbBusinessSpecialBalance"
			scope="request" items="tbBusinessSpecialBalanceList" mode="ajax"
			toolbarPosition="bottom" skin="E3002" pageSize="10" width="1000"
			height="360" caption="特殊编辑单">
