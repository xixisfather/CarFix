...
<%
	List<TbBusinessSpecialBalance> tbBusinessSpecialBalanceList = (List<TbBusinessSpecialBalance>) session
			.getAttribute("tbBusinessSpecialBalanceList");

	TbBusinessSpecialBalance _tbBusinessSpecialBalance = (TbBusinessSpecialBalance)session.getAttribute("tbBusinessSpecialBalance");
%>

<div id="apDiv1">公司特殊结算财务汇总报表&nbsp;&nbsp;<span class="btn"><font
...
<table width="800" height="76" border="0">
	...
	<tr>
		...
		<td><%= tbBusinessSpecialBalance.getEntrustCode() == null ? (tbBusinessSpecialBalance.getStockOutCode() == null ? "" : tbBusinessSpecialBalance.getStockOutCode()) : tbBusinessSpecialBalance.getEntrustCode() %></td>
		...
		<td><%= tbBusinessSpecialBalance.getTbCarInfo() == null ? "" : (tbBusinessSpecialBalance.getTbCarInfo().getLicenseCode())%></td>
		<td><%= tbBusinessSpecialBalance.getTbCarInfo() == null ? "" : (tbBusinessSpecialBalance.getTbCarInfo().getTmCarModelType().getModelCode())%></td>
		<td><%= tbBusinessSpecialBalance.getTbCarInfo() == null ? "" : (tbBusinessSpecialBalance.getTbCarInfo().getChassisCode())%></td>
...
<table width="312" border="0">
	<tr>
		...
		<td width="73"><%=session.getAttribute("totalSize") %></td>
		<td width="73" align="right">总金额：</td>
		<td width="75"><%=session.getAttribute("totalAll") %></td>
	</tr>
</table>
<p>&nbsp;</p>
</div>
