...
//打开配件列表窗口
function showWin(){
	var url = "chooseTbPartInfoLianceAction.action?typeName="+encodeURIComponent(lianceRegTypeName);
...
function createGrid(){
    ...
    grid = new Ext.grid.GridPanel({
        ...
        columns: [
            ...
            {header: "成本价", width: 100, hidden:true,	sortable: true, dataIndex:'costprice'},
            {header: "成本小计", width: 100, hidden:true,sortable: true, dataIndex:'costtotal'},
            {header: "进货价", width: 100, sortable: true, dataIndex:'price'},
            {header: "成本金额小计", width: 100, sortable: true, dataIndex:'pricetotal'},
            {
				header : "操作",
				width : 50,
				dataIndex : 'operation',
...
function addPartCollection(confirm){
	...
	for(var i=0; i<partInfoArray.length; i++){
		...
		var num = partInfoArray[i][partInfoArray[i].length-5];				//数量
		var costprice = partInfoArray[i][partInfoArray[i].length-4];		//成本价
		var price = partInfoArray[i][partInfoArray[i].length-2];			//进货价
		var paramStr = partInfoId+":"+num+":"+costprice+":"+price;
		partCol += paramStr+",";
	}
	...
	if(document.getElementById("peopleId").value == ""){
		Ext.MessageBox.alert('温馨提示：', '必须选择经办人.', null);
		return;
	}
	if( document.getElementById("lianceDate").value == ""){
		Ext.MessageBox.alert('温馨提示：', '借进日期不能为空.', null);
		return;
	}
