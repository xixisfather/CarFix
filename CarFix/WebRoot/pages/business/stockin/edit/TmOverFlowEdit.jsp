...
<html>
	...
	<script type="text/javascript">
		...
		function handlerE3Table(){
			...
			for(var j=0 ; j<tmStockInDetailTableE3ExtStore.getCount(); j++){
				...
				var spanOverFlowXj = document.getElementById("spanOverFlowXj"+partinfoId);					//报溢小计
				
				
				//数量合计
				totalNum = parseFloat(userQuantity.value) + totalNum;
				//税前合计
				totalprice = parseFloat(spanOverFlowXj.innerHTML) + totalprice;
		...
		function jsRecord(stockInDetailId){
			...
			var userCostPrice = document.getElementById("userCostPrice"+stockInDetailId);			//报溢单价
			var spanOverFlowXj = document.getElementById("spanOverFlowXj"+stockInDetailId);			//报溢小计
		
			spanOverFlowXj.innerHTML = Math.round(parseFloat(userQuantity.value)* parseFloat(userCostPrice.value)*100)/100;
		...
		function addRecord(){
			...
			var TopicRecord = Ext.data.Record.create([
			    ...
			    {name: 'storeCostPrice', mapping: 'storeCostPrice'},
			    {name: 'userCostPrice', mapping: 'userCostPrice'},
			    {name: 'overFlowXj', mapping: 'overFlowXj'},
			...
			var myNewRecord = new TopicRecord({	
			    ...
			    storeCostPrice : tbPartInfoObj.price,
			    ...
			    overFlowXj: "<span id=\"spanOverFlowXj"+tbPartInfoObj.tbPartInfoId+"\" >"+tbPartInfoObj.xj+"</span>",
	...
	<body onload="handlerE3Table();" >
		<s:form action="updateOverFlowAction.action" >
		...
		<c:set var="rate" value="1.17" ></c:set>		 
		 ...
		 <e3t:table id="tmStockInDetailTable" uri="initEditStockInPageAction.action" var="detVo"
			...
			<e3t:column property="userCostPrice"  title="报溢单价" >
			...
			<e3t:column property="overFlowXj"   title="报溢小计" >
				<span id="spanOverFlowXj${detVo.partinfoId}" >
					<fmt:formatNumber maxFractionDigits="2"  value="${detVo.quantity * detVo.costPrice }"  ></fmt:formatNumber>
				</span>
			</e3t:column>
