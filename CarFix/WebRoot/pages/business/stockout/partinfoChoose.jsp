...
<html>
	...
	<script type="text/javascript">
		...
		function tbPartInfoTableE3ConfigHandler(pConfig) {
			...
			pConfig.form = "chooseTbPartInfoStockOutAction";
	...
	<body  >
		<s:form action="chooseTbPartInfoStockOutAction.action">
		...
		<e3t:table id="tbPartInfoTable" uri="chooseTbPartInfoStockOutAction.action" var="tbPartInfo"
			...
			<e3t:column property="price" title="${typeName}价" >
				<s:if test="#request.typeName == '销售'">
					<input type="text"  style="width:80px" id="${tbPartInfo.id}price" value="${tbPartInfo.sellPrice}"   />
				</s:if>
				<s:if test="#request.typeName == '领用'">
					<input type="text" style="width:80px" id="${tbPartInfo.id}price" value="${tbPartInfo.costPrice}"   />
				</s:if>
