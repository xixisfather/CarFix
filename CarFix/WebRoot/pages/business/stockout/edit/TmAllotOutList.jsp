	...
	<script type="text/javascript">
		...
		function tmStockOutTableConfigHandler(pConfig) {
			...
			{	
				...
				iconCls : 'addIcon',
				handler : function() {
					//window.location.href = ctx+"/initAllotPageAction.action";
					addObject('initAllotPageAction.action',800,450);
				}
			}, '', '-', '', {
				text : '刷新',
				iconCls : 'refreshIcon',
		...
		function tmStockOutTableE3ConfigHandler(pConfig) {
			...
			pConfig.form = "findAllotOutAction";
			pConfig.showLoadingMsg = true;
		}
		
	</script>
	<body>
		<s:form action="findAllotOutAction.action" >
			<table>
				<tr>
					<td>出库单号：</td>
		...
		<e3t:table id="tmStockOutTable" uri="findAllotOutAction.action" var="stockOut"
			scope="request" items="stockOutList" mode="ajax" caption="调拨单列表"
			...
			<e3t:column property="no" title="操作"
				sortable="false">
				<a href="javascript:editObject('${stockOut.stockOutId}','initEditAllotOutPageAction.action',800,450)">
					<font color="blue">
						修改
					</font>
				</a>
				...
				&nbsp;&nbsp;
				<a href="javascript:editObject('${stockOut.stockOutId}','printDBCKAction.action',600,300);">
					<font color="blue">
						打印
					</font>
				</a>
