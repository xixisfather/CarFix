	...
	<script type="text/javascript">
		...
		function tmStockOutTableConfigHandler(pConfig) {
			...
			{	
				...
				iconCls : 'addIcon',
				handler : function() {
					//window.location.href = ctx+"/initShatterPageAction.action";
					addObject('initShatterPageAction.action',800,450);
				}
			}, '', '-', '', {
				text : '刷新',
				iconCls : 'refreshIcon',
		...
		function tmStockOutTableE3ConfigHandler(pConfig) {
			...
			pConfig.form = "findShatterAction";
			pConfig.showLoadingMsg = true;
		}
		
	</script>
	<body>
		<s:form action="findShatterAction.action" >
			<table>
				<tr>
					<td>出库单号：</td>
		...
		<e3t:table id="tmStockOutTable" uri="findShatterAction.action" var="stockOut"
			scope="request" items="stockOutList" mode="ajax" caption="报损单列表"
			...
			<e3t:column property="no" title="操作"
				sortable="false">
				<a href="javascript:editObject('${stockOut.stockOutId}','initEditShatterPageAction.action',800,450);">
					<font color="blue">
						修改
					</font>
				</a>
				...
				&nbsp;&nbsp;
				<a href="javascript:editObject('${stockOut.stockOutId}','printPJBSAction.action',600,300);">
					<font color="blue">
						打印
					</font>
				</a>
