		...
		<s:form action="tmStockOutDetailShowAction.action" >
			<table>
				<tr>
					...
					<td>
						<input type="text" id="solePartFavourRate" name="solePartFavourRate" onblur="setSolePartFavourRate();" value="${request.solePartFavourRate}"/>
					</td>
					<td>
						合计
					</td>
					<td>
						...
						<input type="text" id="XSJE" name="solePartTotalAll" value="<s:property value="#request.solePartTotalAll"/>" disabled="true"/>
						
						<input type="hidden" id="XSJE_back" name="solePartTotalAll_back" value="<s:property value="#request.solePartTotalAllOri"/>">
...
function setSolePartFavourValue()
{
    
	var solePartFavourRate_temp = document.getElementById('solePartFavourRate');
	
	var solePartFavourRate = parent.document.getElementById('solePartFavourRate');
	
	var XSJE_temp = document.getElementById('XSJE');
	
	var XSJE = parent.document.getElementById('XSJE');
	
	solePartFavourRate.value = solePartFavourRate_temp.value;
	
	XSJE.value = XSJE_temp.value;
}

function setSolePartFavourRate()
{
	var solePartFavourRate_temp = document.getElementById('solePartFavourRate');
	...
	var XSJE_back = document.getElementById('XSJE_back');
	
	clearNoNum22(solePartFavourRate_temp,0,1);
	
	if(isNumber(solePartFavourRate_temp.value))
	{
		XSJE_temp.value = formatFloat((1-solePartFavourRate_temp.value)*XSJE_back.value,2);
	}
	else
	{
		
		XSJE_temp.value = XSJE_back.value;
	}
	
	setSolePartFavourValue();
}
