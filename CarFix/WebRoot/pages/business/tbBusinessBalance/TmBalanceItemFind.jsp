...
<html>
	<head>
  	...
  	<body>
    	<s:form action="tmBalanceItemCalcAction.action">
			<s:hidden id="tmBalanceId" name="tmBalanceItem.tmBalance.id"/>
		</s:form>
		<e3t:table id="tmBalanceItemTable" uri="tmBalanceItemCalcAction.action" var="tmBalanceItem"
			...
			<e3t:column property="no" title="项目金额">
				...
				<c:choose>
               		<c:when test="${tmBalanceItem.allowHand == 1}">
                		<input type="text" id="${tmBalanceItem.itemCode}" name="${tmBalanceItem.itemCode}" value="0" onblur="calcAjax('${tmBalanceItem.itemCode}')"/><font color="red">*</font>
              		</c:when>
               		<c:when test="${tmBalanceItem.allowHand == 0}">
               			<input type="text" id="${tmBalanceItem.itemCode}" name="${tmBalanceItem.itemCode}" value="0" disabled="true"/>
               		</c:when>
		...
		<script type="text/javascript" src="<%=request.getContextPath() %>/pages/business/tbBusinessBalance/TmBalanceItem.js" charset="UTF-8"></script>
...
<script language="javascript">
	...
	{
	<%
		Map calcMapReturn = (LinkedHashMap)session.getAttribute("calcMapReturn");
