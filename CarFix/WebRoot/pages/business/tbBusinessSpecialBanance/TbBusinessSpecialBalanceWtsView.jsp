		...
		<s:form action="tbBusinessSpecialBalanceUpdateAction.action">
			<table>
				...
				<tr>	
					...
					<td>
						<s:textfield name="tbBusinessSpecialBalance.tmUser.userRealName" readonly="true"></s:textfield>
						<s:hidden id="tmUserId" name="tbBusinessSpecialBalance.tmUser.id"></s:hidden>
					</td>
					<td>
						备注
					</td>
					<td>
						<s:textfield size="50" name="tbBusinessSpecialBalance.remark" readonly="true"></s:textfield>
					</td>
				</tr>
				<tr>
							...
							<s:if test="#request.tbSpecialWorkingContentList!=null">
								...
								<s:iterator id="tbFixEntrustContent" value="#request.tbSpecialWorkingContentList" status="status">
									...
									<tr style="display: none">
										<td><input type="text" id="workingHourPrice" name="workingHourPrice" value="${tbFixEntrustContent.workingHourPrice}" disabled="true"></td>
									</tr>
									</s:if>
									<tr>
										<td>
										...
										<td>
											<s:if test="#tbFixEntrustContent.freesymbol==1">
												<input type="checkbox" id="freesymbol${status.index}" name="freesymbol${status.index}" checked="checked" value="1" disabled="true"/>
											</s:if>
											<s:else>
												<input type="checkbox" id="freesymbol${status.index}" name="freesymbol${status.index}" disabled="true"/>
											</s:else>
										</td>
										...
										<td><input type="text" id="fixHourAll${status.index}" name="fixHourAll${status.index}" value="${tbFixEntrustContent.fixHourAll}" disabled="true"></td>
									</tr>
									...
									<tr>
										...
										<td>
											...
											<s:if test="#sellStock.isFree==1">
												<input type="checkbox" id="free${status.index}" name="free${status.index}" checked="checked" value="1" disabled="true"/>
											</s:if>
											<s:else>
												<input type="checkbox" id="free${status.index}" name="free${status.index}" disabled="true"/>
											</s:else>
										</td>
										...
										<td>
											<s:if test="#sellStock.partNameDB==null">
												...
												<input type="text" id="partName${status.index}" name="partName${status.index}" value="${sellStock.tbPartInfo.partName}" disabled="true"/>
											</s:if>
											<s:else>
										...
										<td><input type="text" id="total${status.index}" name="total${status.index}" value="${sellStock.partTotal}" disabled="true"/></td>
									</tr>
								</s:iterator>
				...
				<tr>
					...
					<c:choose>
						    <c:when test="${request.comName=='dfbz'}">
							    <input type="button" value="打印东风标致" onclick="javascript:window.open('tbBusinessSpecialBalanceTemplatePrintAction.action?id=${tbBusinessSpecialBalance.id}&companyName=dfbz')">
						    </c:when>
						    
						    <c:when test="${request.comName=='xtl'}">
							    <input type="button" value="打印雪条龙" onclick="javascript:window.open('tbBusinessSpecialBalanceTemplatePrintAction.action?id=${tbBusinessSpecialBalance.id}&companyName=xtl')">
						    </c:when>
						     
						    <c:otherwise>
						         <input type="button" value="打印" onclick="javascript:editObject('${tbBusinessSpecialBalance.id}','tbBusinessSpecialBalanceWtsPrintAction.action',600,300);"/>
						...
						<!--<input type="button" value="返回结算" onclick="redirectPage();"/>-->
					</td>
...
Ext.onReady(function(){
	...
	function redirectPage(){
		...
		window.location.href = 'pages/business/tbBusinessSpecialBalance/tbBusinessBalanceSelectAction.action';
	}
</script>
